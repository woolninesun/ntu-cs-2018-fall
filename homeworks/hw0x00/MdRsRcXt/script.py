...
m = int( '0x' + bytes.decode( binascii.b2a_hex( bytes(m) ) ).lstrip("0"), 16 )
...
x = egcd(65537, (b-1)*(a-1))
m = hex( pow(m, x + (b-1)*(a-1), b*a) )[2:]
...
chrmap = {}
for w in range(128):
    chrmap[md5(chr(w))] = chr(w)

FLAG = ""
for i in range(0, len(m), 32):
    FLAG += chrmap[ m[i:i+32] ]
print (FLAG)
